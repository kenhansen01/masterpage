...
var navTiles = function($, restUtils) {
    ...
    var navTilesItems = function() {
            ...
            if(selectedItems.results.length === 0)
                ...
                for(var i = 0; i < items.results.length; i++) {
                    ...
                    var item            =    items.results[i],
                        ...
                        itemContainer   =    $('<a class="dbs-nav-tile col-md-4 col-xs-6" href="'
                                              + itemLink +'" />'),
                        itemNavBlock    =    $('<div class="dbs-nav-block" />'),
                        itemNavContent  =    $('<div class="dbs-nav-content" />'),
                        itemIcon        =    $('<div class="dbs-nav-icon">'
                                              +    '<img src="'+ itemIcon +'" alt="" />'
                                              +'</div>'),
                        itemTitle       =    $('<div class="dbs-nav-title"><h2>'+ itemName +'</h2></div>');
        ...
        var _getAvailableItems = function(listTitle) {            
            var baseUrl = _spPageContextInfo.siteAbsoluteUrl + "/_api/web/lists/getbytitle('"+listTitle+"')/getitems",
                ...
                /*success           =    function(data) {
                                         window.DBS.NavTileItems.availableItemsObj = JSON.parse(data.body);
                                         defObj.resolve(window.DBS.NavTileItems.availableItemsObj);
                                       },
        ...
        var _setSelectedItems = function(availableItems) {
            ...
            $('#dbs-icon-json textarea').text(sItemsJSON);
    ...
    var init = function() {
        var _listTitle         = 'Site Icon Nav Buttons',
            iconEditTextArea   = $('#dbs-icon-json textarea'),
            items              = navTilesItems(),
            sort               = sorting();
        ...
        if(iconEditTextArea.length > 0) {        
            ...
            $.when(selectedItems, availableItems).done(function(sdata, adata){
                ...
                var sItemsObj  = sdata[0].d,
                    selected   = JSON.parse(sItemsObj.DBS_x0020_Page_x0020_Icons_x0020_JSON_x0020_Object),
